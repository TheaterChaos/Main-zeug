spawn_vehicle_at_position = function(model_name, position, heading)
...
local function spawn_item(target)
    ...
    local camera_rotation = GET_FINAL_RENDERED_CAM_ROT(2)
    spawn_vehicle_at_position(item.model, target.pos, camera_rotation.z)
end
...
local function build_vehicles_items()
    ...
    for _, vehicle in pairs(util.get_vehicles()) do
        local item = {
            name = util.get_label_text(GET_DISPLAY_NAME_FROM_VEHICLE_MODEL(util.joaat(vehicle.name))),
            model = vehicle.name,
